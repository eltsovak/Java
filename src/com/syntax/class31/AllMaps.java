...
public class AllMaps {
	...
	public static void main(String[] args) {
		...
		mall.put("Colone", 99.99); // K + V = entry
		...
		// shoppingList.put(null, 10.99); -> Exception
		// can have null value, but NO null key
		
		System.out.println(shoppingList);
		
		Map<String, Double> htable = new Hashtable<>();
		htable.put("House", 600000.00);
		htable.put("Car", 65000.00);
		htable.put("Cup", 10.0);
		htable.put("Food", 100.0);
		//htable.put(null, 1.99); -> Hashtable does not allow duplicate keys
		//htable.put("Milk", null);  -> Hashtable does not allow duplicate values
		
		System.out.println(htable);
		
		// adding null keys and null values to the Hashmap
		grocery.put(null, 0.00); // HashMap can have at least one null key
